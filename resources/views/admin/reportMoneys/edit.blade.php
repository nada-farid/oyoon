...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.reportMoney.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.report-moneys.update", [$reportMoney->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('part') ? 'is-invalid' : '' }}" name="part" id="part" required>
                    ...
                    @foreach(App\Models\ReportMoney::PART_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('part', $reportMoney->part) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('part'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number" name="year" id="year" value="{{ old('year', $reportMoney->year) }}" step="1" required>
                @if($errors->has('year'))
                    <div class="invalid-feedback">
                        {{ $errors->first('year') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $reportMoney->link) }}">
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
