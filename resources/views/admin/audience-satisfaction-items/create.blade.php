...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} عنصر قياس رضا الجمهور
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.audience-satisfaction-items.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('audience_satisfaction') ? 'is-invalid' : '' }}" name="audience_satisfaction_id" id="audience_satisfaction_id" required>
                    @foreach($audienceSatisfactions as $id => $entry)
                        <option value="{{ $id }}" {{ old('audience_satisfaction_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('published') ? 'is-invalid' : '' }}" type="checkbox" name="published" id="published" value="1" {{ old('published', '0') == '1' ? 'checked' : '' }}>
